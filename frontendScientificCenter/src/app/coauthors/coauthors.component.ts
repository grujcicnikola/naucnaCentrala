 ...
 /* private scarea11: any;
  ...
  onSubmit(value, form){
    ...
    for (var property in value) {
      ...
      if(property =="cityCoauthor"){
        adresa +="+"+value[property].replace(" ", "+");
      }
      if(property =="stateCoauthor"){
        adresa +="+"+value[property].replace(" ", "+");
       }
    ...
    console.log(o);
    this.taskService.findLongitudeAndLatitude(adresa).subscribe(response => {
      var lon = response[0].lon;
      var lat = response[0].lat;
      console.log(lon)
      console.log(lat)
      
      let x = this.taskService.registerCoauthors(o, this.formFieldsDto.taskId,lon,lat);
              x.subscribe(
                res => {
                  console.log(res);
